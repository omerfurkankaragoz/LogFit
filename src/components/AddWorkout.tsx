...
const AddWorkout: React.FC<AddWorkoutProps> = ({ date, onSave, onCancel, selectedExercise }) => {
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-6">
        <h3 className="text-sm font-medium text-gray-600 mb-2">Popüler Hareketler</h3>
        ...
        {exercises.map((exercise, exerciseIndex) => (
          <div key={exercise.id} className="bg-white rounded-xl p-4 shadow-sm">
              ...
              {exercise.sets.map((set, setIndex) => (
                <div key={setIndex} className="grid grid-cols-4 gap-2 items-center">
                  ...
                  <input
                    ...
                    placeholder="0"
                    className="p-2 border border-gray-200 rounded-lg text-center focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                  <input
                    type="number"
                    value={set.weight || ''}
            ...
            <button
              ...
              className="w-full mt-3 p-2 text-blue-600 border-2 border-blue-200 border-dashed rounded-lg hover:bg-blue-50 transition-colors"
            >
              + Set Ekle
            </button>
          </div>
      ...
      <button
        onClick={addExercise}
        className="w-full mt-6 p-4 bg-gray-100 text-gray-600 rounded-xl border-2 border-gray-200 border-dashed hover:bg-gray-200 transition-colors flex items-center justify-center gap-2"
      >
        <Plus size={20} />
        Hareket Ekle
      </button>
